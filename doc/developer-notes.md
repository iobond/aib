...
Run with the -testnet option to run with "play aibs" on the test network, if you
...
- BitcoinMiner : Generates aibs (if wallet is enabled).
...
- src/libsecp256k1
  - Upstream at https://github.com/aib-core/secp256k1/ ; actively maintaned by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/aib-core/ctaes ; actively maintained by Core contributors.
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:aib/aib.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `aib-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `aib-cli` and the GUI debug console use this table to determine how to
