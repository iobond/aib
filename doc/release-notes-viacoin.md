Aibcoin Core version 0.13.2 is now available from:

  <https://github.com/aibcoin/aibcoin/releases>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/aibcoin/aibcoin/issues>
...
updates will be released anymore. Without security updates, using a aibcoin
...
ECDSA signatures inside Aibcoin transactions now use validation using
...
Previous versions of Aibcoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Aibcoin Core was to raise the default minimum
relay fee.

Aibcoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Aibcoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running aibcoind without having to do any manual configuration.
...
Aibcoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Aibcoin Core will relay transactions with
...
entirely in Aibcoin Core 0.13, and it is at this time undecided whether the
...
Aibcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Aibcoin Core automatically creates a hidden service to listen on, without
manual configuration. Aibcoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Aibcoin Core is listening, and
...
Aibcoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Aibcoin
Core to use floating fees. By default, Aibcoin Core will use floating
fees.
...
(default: `0.0002` AIB/kB).

At all times, Aibcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) AIB.
Furthermore, Aibcoin Core will never create transactions paying less than
...
`aibcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /aibcoinrpc>
        ...
        # optional bypass aibcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< aibcoinrpc:<password>
    </Location>
...
Restarting aibcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in aibcoin.conf
...
For example:
    ...
    $ src/aibcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Aibcoin Core code base has
...
- `aibcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `aibcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Aibcoin scripting system that in combination with
...
Aibcoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Aibcoin Core.
...
The two phases are distinct in the Aibcoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Aibcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Aibcoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Aibcoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of aibcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of aibcoins being spent is exactly
...
- **Increased security for multisig:** Aibcoin addresses (both P2PKH addresses
  ...
  believed to be provided by Aibcoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Aibcoin's current ledger (called the UTXO set),
  ...
  note that Aibcoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Aibcoin users to individually opt-in to almost any change in the Aibcoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Aibcoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Aibcoin
client to Aibcoin Core 0.13.2 to signal for segwit support. When a
...
Since Aibcoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `aibcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `aibcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
(or in the GUI aib a context menu on the transaction).
