...
#if defined(NDEBUG)
#error "Aib cannot be compiled without assertions."
#endif
...
//TODO: AIB MERGE copay address index
...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree,
...

    {
        ...
        LockPoints lp;
        {
            ...
            BOOST_FOREACH(const CTxIn txin, tx.vin) {
                ...
                if (!view.HaveCoins(txin.prevout.hash)) {
                    LogPrintf("NOT passing HaveCoins Check\n");
                    if (pfMissingInputs)
            ...
            if (!view.HaveInputs(tx)) {
                LogPrintf("NOT passing HaveInputs Check\n");
        ...
        BOOST_FOREACH(const CTxMemPool::txiter it, allConflicting) {
            LogPrint("mempool", "replacing tx %s with %s for %s AIB additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams) {
    return GetBlockSubsidy(nHeight, consensusParams, 0);
}

CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, CAmount nFees) {
    //TODO: AIB REGTEST
    ...
    if (nHeight == 0) {
        ...
        // first distribution
        // TODO: AIB RECHECK
        nSubsidy = 41092653589793238;
        //10000000 * COIN;
    } else if (nHeight > 1 && nHeight <= 9) {
        nSubsidy = 387500000000000000;
    } else
...
namespace Consensus {
    ...
    bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight) {
        ...
        //TODO: AIB SPEC
        static const int nLeagcyMaturity = WTMINT_COINBASE_MATURITY;
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            if (coins->IsCoinBase()) {
                //TODO: AIB SPEC
                int validationMaturity = nMaturity;
                if (nSpendHeight < WTMINT_LEAGCY_BEST_BLOCK_HEIGHT )
                    validationMaturity = nLeagcyMaturity;
                if (nSpendHeight - coins->nHeight < validationMaturity)
                    return state.Invalid(false,
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex,
    ...
    if (fAddressIndex) {
        if (addressIndex.size() && !pblocktree->WriteAddressIndex(addressIndex)) {
            return AbortNode(state, "Failed to write address index");
        }

        if (addressUnspentIndex.size() && !pblocktree->UpdateAddressUnspentIndex(addressUnspentIndex)) {
            return AbortNode(state, "Failed to write address unspent index");
        }
    }
    if (fSpentIndex)
        if (spentIndex.size() && !pblocktree->UpdateSpentIndex(spentIndex))
    ...
    while (fContinue && nHeight != pindexMostWork->nHeight) {
        ...
        BOOST_REVERSE_FOREACH(CBlockIndex *pindexConnect, vpindexToConnect) {
            ...
            if (!ConnectTip(state, chainparams, pindexConnect, pindexConnect == pindexMostWork ? pblock : NULL)) { 
                   // || pblock->GetHash().GetHex()=="0x062550537871c387faaa5cd91bb8a2cf4b67d3a4ec816799ecc1f5cb7fd54bcc") {
                if (state.IsInvalid()) {
    ...
    if (!fKnown) {
        ...
        if (nNewChunks > nOldChunks) {
            ...
            if (CheckDiskSpace(nNewChunks * BLOCKFILE_CHUNK_SIZE - pos.nPos)) {
                ...
                LogPrintf("Error: out of disk space");
                return state.Error("out of disk space");
            }
        }
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime) {
    ...
    if ((consensusParams.fPowAllowMinDifficultyBlocks && nHeight < 300000) || nHeight < 451000) {
        timeframe = 60 * 60; //15 * 60;
    } else {
        timeframe = 30 * 60; //5 * 60;
    }
    ...
    if (!Params().MineBlocksOnDemand() && (consensusParams.fPowAllowMinDifficultyBlocks || nHeight >= 100)) {
        if (block.GetBlockTime() > GetAdjustedTime() + timeframe) {
            unsigned long int diff = block.GetBlockTime() - ( GetAdjustedTime() + timeframe );
            return error("AcceptBlock() : block's timestamp too far: %d in the future for %d time frame", diff, timeframe);
        }

        // Check timestamp is not too far in the past (timewarp)
        if (block.GetBlockTime() <= pindexPrev->GetBlockTime() - timeframe) {
            unsigned long int diff = block.GetBlockTime() - ( pindexPrev->GetBlockTime() - timeframe );
            return error("AcceptBlock() : block's timestamp is too early: %d compare to last block for %d time frame", diff, timeframe);
        }
    }
    ...
    /*for (int32_t version = 4; version <= 6; ++version) // Aib check for version 3, 4 and 5 upgrades
        if ((block.nVersion & 0xFF) < VERSIONBITS_TOP_BITS
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev) {
    ...
    //TODO: AIB REGTEST
    // AIB: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
    // Enforce block.nVersion=2 rule that the coinbase starts with serialized block height
    // if 750 of the last 1,000 blocks are version 2 or greater (51/100 if testnet):
    bool checkHeightMismatch = false;

    // TODO: AIB MERGE RECHECK aib block has a "cooked" version number which needs to be revert to standard version number
...
static bool IsSuperMajority(int minVersion, const CBlockIndex* pstart, unsigned nRequired, const Consensus::Params& consensusParams) {
    ...
    for (int i = 0; i < consensusParams.nMajorityWindow && nFound < nRequired && pstart != NULL; i++) {
        //TODO: AIB AUX
        // Aib mask off the Chain_ID and AuxPow version
...
bool static LoadBlockIndexDB() {
    ...
    LogPrintf("%s: transaction index %s\n", __func__, fTxIndex ? "enabled" : "disabled");    // Check whether we have a timestamp index    //pblocktree->ReadFlag("timestampindex", fTimestampIndex);
	///////////////////////////////////////////////////////////////////////////////////////////////
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth) {
    ...
    if (nCheckDepth <= 0)
        //TODO: AIB SPEC
        // Aib: Didn't 25x value due to integer wrap around
