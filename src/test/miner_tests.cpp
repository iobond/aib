...
BOOST_AUTO_TEST_CASE(CreateNewBlock_validity)
{
    ...
    for (unsigned int i = 0; i < sizeof(blockinfo)/sizeof(*blockinfo); ++i)
    {
        // Aib: Begin
        if (i == 1 || i >= 3599) {
            // Aib needs coinbase value reset after first block; diff change starts at block 3600
        ...
        txCoinbase.vin[0].scriptSig.push_back(chainActive.Height() >> 8); // Aib
    ...
    // Aib has zero reward in genesys, unspendable
    // BOOST_CHECK(pblocktemplate = BlockAssembler(chainparams).CreateNewBlock(scriptPubKey));
    pblocktemplate.reset();
    mempool.clear();
    ...
    // Aib cannot spend multiple inputs as starting blocks have a single non-zero one
/*
    BOOST_CHECK(pblocktemplate = BlockAssembler(chainparams).CreateNewBlock(scriptPubKey));
