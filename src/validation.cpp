...
const std::string strMessageMagic = "AIB Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight == 0) {
        ...
        // first distribution
        // TODO: AIB Merge Recheck Major ConnectBlock #001
        nSubsidy = WTMINT_MAX_SEND;
    } else if (nHeight > 1 && nHeight <= 9) {
        nSubsidy = WTMINT_MAX_MONEY;
    } else {
        // TODO: AIB Merge Recheck Critical BlockConnect #002
        // nFees might needed somewhere else need to confirm later
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    // TODO: AIB Merge Recheck Segwit #001
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // TODO: AIB Merge Recheck Normal BIP #001
    // Start enforcing BIP68 (sequence locks) and BIP112 (CHECKSEQUENCEVERIFY) using versionbits logic.
    ...
    try {
    {
        ...
        if (fDoFullFlush || fPeriodicWrite) {
            ...
            // Then update all block file information (which may refer to block and undo files).
            {
                ...
                //TODO: AIB Merge Recheck Major #auxpow 001
                if (!pblocktree->WriteBatchSync(vFiles, nLastBlockFile, vBlocks, mapDirtyAuxPow)) {
                    return AbortNode(state, "Failed to write to block index database");
                }
                // Remove corresponding vblocks from auxpow mapping
...
{
    {
        ...
        if (nBlockReverseSequenceId > std::numeric_limits<int32_t>::min()) {
            ...
            // call  2**31-1 times on the same set of tips...
...
CBlockIndex* CChainState::AddToBlockIndex(const CBlockHeader& block)
{
    ...
    //add block to auxpow mapping to sync with dirty block insertion
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    // replace with check block proof of work for auxpow
...
bool IsWitnessEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params)
{
    ...
    // TODO: AIB Merge Recheck Minor
    // if block height greater than witness start height, return enabled.
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // TODO: AIB Merge Recheck Minor Auxpow #002
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // TODO: AIB Merge Recheck Normal Witness # 002
    ...
    // TODO: AIB Merge Recheck Minor Block #001 
    // block version check >= 2
...
void UnloadBlockIndex()
{
    ...
    // TODO: AIB Merge Recheck Auxpow #003
    // clear auxpow mapping
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                //LogPrint(BCLog::REINDEX," loop # %d: current block %s nbits %ld \n", i, hash.GetHex().c_str(), block.nBits);
                {
                    LOCK(cs_main);
                    // detect out of order blocks, and store them for later
                    
                    if (hash != chainparams.GetConsensus().hashGenesisBlock && !LookupBlockIndex(block.hashPrevBlock)) {
                        // TODO: AIB Merge Restore to REINDEX
                        //LogPrint(BCLog::ALL, "%s: Out of order block %s, parent %s not known\n", __func__, hash.ToString(),
                ...
                //LogPrint(BCLog::REINDEX,"loop # %d: hash = %s genesis hash = %s \n", i, hash.GetHex().c_str(), chainparams.GetConsensus().hashGenesisBlock.GetHex().c_str());
                if (hash == chainparams.GetConsensus().hashGenesisBlock) {
                    //LogPrint(BCLog::REINDEX,"loop # %d: hash = %s genesis hash = %s \n", i, hash.GetHex().c_str(), chainparams.GetConsensus().hashGenesisBlock.GetHex().c_str());
                    CValidationState state;
                    if (!ActivateBestChain(state, chainparams)) {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        {
                            // TODO: AIB Merge Restore to REINDEX
                            //LogPrint(BCLog::ALL, "%s: Processing out of order child %s of %s\n", __func__, pblockrecursive->GetHash().ToString(),
                            //        head.ToString());
                            LOCK(cs_main);
